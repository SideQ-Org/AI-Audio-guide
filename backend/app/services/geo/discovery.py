...
from .ranking import Dedup, build_candidates
...
class Discovery:
    ...
    async def discover(
        ...
        return build_candidates(
            position, heading, places, radius_m, seen, dedup
        )
    ...
    async def discover_adaptive(
        ...
        candidates = await self.discover(position, heading, seen, radius_m, dedup)
        if candidates:
        ...
        candidates = await self.discover(position, heading, seen, wide, dedup)
    ...
    async def discover_inventory(
        ...
        if not inv.places:
            ...
            return await self.discover_adaptive(
                position, heading, seen, settings.default_radius_m, dedup
            )
        candidates = build_candidates(
            position, heading, inv.places, settings.default_radius_m, seen, dedup
        )
        if candidates:
            log.debug(
                "inventory: disc=%d cands@%.0fm=%d (of which unseen ranked)",
                len(inv.places), settings.default_radius_m, len(candidates),
            )
        ...
        wide = build_candidates(
            position, heading, inv.places, settings.inventory_radius_m, seen, dedup
        )
        ahead = [c for c in wide if c.place.id not in passed]
        candidates = ahead or wide
        log.info(
            "inventory sparse: 0 within %.0fm -> widen %.0fm | disc=%d wide=%d passed=%d ahead=%d",
            settings.default_radius_m, settings.inventory_radius_m,
            len(inv.places), len(wide), len(passed), len(ahead),
        )
        return DiscoveryResult(
